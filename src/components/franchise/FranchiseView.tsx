      ...
      <WatchProgressProvider slug={slug}>
        <div className="min-w-0 lg:flex lg:items-start lg:gap-10">
          <aside className="lg:sticky lg:top-28 lg:w-[330px] lg:shrink-0">
            {backdrop && (
              <div className="relative aspect-[16/9] w-full lg:overflow-hidden lg:rounded-2xl">
                <Image
                  src={backdrop}
                  alt={name}
                  fill
                ...
                />
                <div className="from-cinema-bg via-cinema-bg/70 absolute inset-0 bg-gradient-to-t to-transparent lg:via-transparent" />
              </div>
            )}

            <div
              className={backdrop ? "relative -mt-6 px-4 lg:mt-4 lg:px-0" : "px-4 pt-5 lg:px-0"}
            >
              <p className="text-cinema-accent max-w-full text-[11px] font-semibold uppercase tracking-widest">
                Ruta de visualización
              </p>
              <h1 className="mt-1 max-w-full text-2xl font-bold tracking-tight break-words lg:text-3xl">
                {name}
              </h1>
              {tagline && <p className="text-cinema-muted mt-1 text-sm">{tagline}</p>}
