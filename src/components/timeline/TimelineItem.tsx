...
import type { TimelineDensity } from "@/components/watch/WatchProgressProvider";
...
export function TimelineItem({
  ...
  return (
    <li className={cn("relative pl-12", compact ? "pb-1.5" : "pb-3 lg:pb-4")}>
      {!isLast && (
        <span
          aria-hidden
          className={cn(
            "absolute bottom-0 left-[19px] w-0.5",
      ...
      <button
        ...
        className={cn(
          "focus-visible:ring-cinema-accent absolute left-0 flex items-center justify-center rounded-full border-2 text-xs font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 active:scale-95",
          compact ? "top-1 size-8" : "top-2 size-10",
          watched
      ...
      <details className="group bg-cinema-surface border-cinema-border hover:border-cinema-muted/40 overflow-hidden rounded-2xl border transition-colors">
        <summary
          className={cn(
            "flex cursor-pointer items-center active:opacity-80",
            compact ? "gap-2 px-3 py-2" : "gap-3 p-3 lg:gap-4 lg:p-4",
          )}
        >
          {!compact && (
            <Poster
          ...
          <div className="min-w-0 flex-1">
            <p
              className={cn(
                ...
                compact ? "truncate text-sm font-medium" : "text-sm lg:text-base",
            ...
            {compact ? (
              <p className="text-cinema-muted text-xs tabular-nums">{year}</p>
            ) : (
          ...
          <ChevronDownIcon
            className={cn(
              "text-cinema-muted shrink-0 transition-transform group-open:rotate-180",
